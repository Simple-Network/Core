...
export default class AdapterPipeline {
	...
	private _adapters: Array<NamedAdapter<WriteAdapter<any, any> | ReadAdapter<any, any> | ExceptionHandler>> = []

	public addLast<IN, OUT>(name: string, adapter: ReadAdapter<IN, OUT> | WriteAdapter<IN, OUT> | ExceptionHandler) {
		this._adapters.push(new NamedAdapter(name, adapter))
	}

	public addFirst<IN, OUT>(name: string, adapter: ReadAdapter<IN, OUT> | WriteAdapter<IN, OUT> | ExceptionHandler) {
		this._adapters.splice(0, 0, new NamedAdapter(name, adapter))
	}

	public addBefore<IN, OUT>(beforeName: string, name: string, adapter: ReadAdapter<IN, OUT> | WriteAdapter<IN, OUT> | ExceptionHandler) {
	...
	public addAfter<IN, OUT>(beforeName: string, name: string, adapter: ReadAdapter<IN, OUT> | WriteAdapter<IN, OUT> | ExceptionHandler) {
		const named = new NamedAdapter(name, adapter)
		const index = this.indexOf(beforeName)
	...
	public removeAdapter(name: string): WriteAdapter<any, any> | ReadAdapter<any, any> | ExceptionHandler {
